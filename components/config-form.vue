<template>
    <div
        class="responsive-padding p-6 container mx-auto px-4 bg-background text-text"
    >
        ...
        <form>
            <div class="mb-4">
                ...
                <label
                    for="os"
                    class="block text-text text-sm font-bold mb-2 mt-4"
            ...
            <div class="mb-4">
                ...
                <input
                    ...
                    v-model="relayName"
                    @input="validateRelayName($event)"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-text leading-tight focus:outline-none focus:shadow-outline"
                    placeholder="Enter Relay Nickname"
                    required
                />
                <span v-if="!isRelayNameValid" class="text-red-500 text-xs"
                    >Invalid relay name. Must be 1-19 characters long and
            ...
            <div class="mb-4 flex items-center">
                <input
                    id="enable-nyx-monitoring"
                    type="checkbox"
                    name="enable-nyx-monitoring"
                    class="ml-4 mr-2 leading-tight"
                    v-model="enableNyxMonitoring"
                />
                <label for="enable-nyx-monitoring" class="text-text"
                    >Enable Nyx monitoring</label
                >
                <input
                    id="blockbadips"
                    type="checkbox"
            ...
            <div class="mb-4 flex">
                <div class="flex-1 mr-2">
                    ...
                    <input
                        ...
                        max="65535"
                        v-model="orPort"
                        @input="validatePort($event)"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-text leading-tight focus:outline-none focus:shadow-outline"
                        placeholder="9001"
                        required
                    />
                    <span v-if="!isOrPortValid" class="text-red-500 text-xs"
                        >Invalid port number. Must be a number between 3000 and
                        65535.</span
                    >
                </div>
                <div class="flex-1 mb-4" v-if="nodeType === 'exit'">
                    ...
                    <input
                        ...
                        placeholder="9030"
                    />
                    <span v-if="!isDirPortValid" class="text-red-500 text-xs"
                        >Invalid port number. Must be a number between 3000 and
                        65535.</span
                    >
                </div>
                <div class="flex-1 mb-4" v-if="nodeType === 'bridge'">
                    ...
                    <input
                        ...
                        required
                    />
                    <span v-if="!isObfs4PortValid" class="text-red-500 text-xs"
                        >Invalid port number. Must be a number between 3000 and
                        65535.</span
                    >
                </div>
            ...
            <div class="mb-4">
                <label
                    for="trafficLimit"
                    class="block text-text text-sm font-bold mb-2"
                    >Total (Up + Down) monthly traffic limit. Type nolimit for
                    No Limit</label
                >
                <input
                    id="trafficLimit"
                    type="text"
                    v-model="trafficLimit"
                    @input="validateTrafficLimit($event)"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-text leading-tight focus:outline-none focus:shadow-outline"
                    placeholder="e.g.  10TB"
                />
                <span v-if="!isTrafficLimitValid" class="text-red-500 text-xs"
                    >Invalid traffic limit format. Must be a number followed by
                    TB, GB, or MB.</span
                >
            </div>
            <div class="mb-4 flex">
                <div class="flex-1 mr-2">
                    <label
                        for="maxBandwidth"
                        class="block text-text text-sm font-bold mb-2"
                        >Maximum bandwidth. Type nolimit for No Limit</label
                    >
                    <input
                        id="maxBandwidth"
                        type="text"
                        v-model="maxBandwidth"
                        @input="validateBandwidth($event)"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-text leading-tight focus:outline-none focus:shadow-outline"
                        placeholder="Value in Mb/s"
                    />
                    <span
                        v-if="!isMaxBandwidthValid"
                        class="text-red-500 text-xs"
                        >Invalid bandwidth format. Must be a number.</span
                    >
                </div>
                <div class="flex-1">
                    <label
                        for="maxBurstBandwidth"
                        class="block text-text text-sm font-bold mb-2"
                        >Maximum burst bandwidth. Type nolimit for No
                        Limit</label
                    >
                    <input
                        id="maxBurstBandwidth"
                        type="text"
                        v-model="maxBurstBandwidth"
                        @input="validateBandwidth($event)"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-text leading-tight focus:outline-none focus:shadow-outline"
                        placeholder="Value in Mb/s"
                    />
                    <span
                        v-if="!isMaxBurstBandwidthValid"
                        class="text-red-500 text-xs"
                        >Invalid burst bandwidth format. Must be a number.</span
                    >
                </div>
            </div>
        </form>

        <h2 class="p-2 text-2xl font-bold mb-4 justify-center text-center">
...
export default {
    ...
    methods: {
        validatePort(event) {
            ...
            const regex = /^\d+$/;
            const isValid =
                regex.test(value) && value >= 3000 && value <= 65535;
